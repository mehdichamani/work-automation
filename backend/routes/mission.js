...
module.exports = function () {
  ...
  async function addHistory(id, userId, userName, action, comment) { await addHistoryHelper('mission_history', 'request_id', id, userId, userName, action, comment); }
  async function findSupervisorId(departmentId) { return findSupervisorIdHelper(departmentId); }
  async function getNextNumber() { return getNextNumberHelper('mission_counter', 'ماموریت'); }
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      const total = await prisma.missionRequest.count({ where });
      const rows = await prisma.missionRequest.findMany({
        where,
  ...
  router.get('/:id', async (req, res) => {
    try {
      const request = await prisma.missionRequest.findUnique({
  ...
  router.post('/', validateInput({ destination: 200, description: 1000 }), mission, async (req, res) => {
    try {
      ...
      const result = await prisma.missionRequest.create({
        data: {
          userId: req.user.id,
          requestNumber,
      ...
      if (supervisorId) {
        await notify(supervisorId, 'درخواست ماموریت جدید', `درخواست شماره ${requestNumber} توسط ${req.user.full_name} ثبت شد`, '/mission');
      }
  ...
  router.post('/:id/approve', async (req, res) => {
    try {
      ...
      const request = await prisma.missionRequest.findUnique({ where: { id: Number(req.params.id) } });
      if (!request) return res.status(404).json({ error: 'درخواست یافت نشد' });
      ...
      if (request.status === 'pending_supervisor') {
        ...
        for (const mgr of managers) {
          await notify(mgr.id, 'درخواست ماموریت نیاز به تایید', `درخواست شماره ${request.request_number} توسط سرپرست تایید شد`, '/mission');
        }
      ...
      await notify(request.userId, 'تایید درخواست ماموریت', `درخواست شماره ${request.request_number} تایید شد`, '/mission');
  ...
  router.post('/:id/reject', async (req, res) => {
    try {
      ...
      await notify(request.userId, 'رد درخواست ماموریت', `درخواست شماره ${request.request_number} رد شد`, '/mission');
  ...
  router.delete('/:id', async (req, res) => {
    try {
      ...
      await prisma.missionHistory.deleteMany({ where: { requestId: Number(req.params.id) } });
      await prisma.missionRequest.delete({ where: { id: Number(req.params.id) } });
