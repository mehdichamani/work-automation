...
module.exports = function (db) {
  ...
  function addHistory(id, userId, userName, action, comment) { addHistoryHelper(db, 'project_supply_requests_history', 'request_id', id, userId, userName, action, comment); }
  function findSupervisorId(departmentId) { return findSupervisorIdHelper(db, departmentId); }
  function getNextNumber() { return getNextNumberHelper(db, 'project_supply_requests_counter', 'پروژه'); }
  ...
  router.get('/', (req, res) => {
    try {
      ...
      const total = db.prepare(`SELECT COUNT(*) as count FROM project_supply_requests_requests p ${where}`).get(...params).count;
      const requests = db.prepare(`
        ...
        FROM project_supply_requests_requests p
  ...
  router.get('/:id', (req, res) => {
    try {
      ...
      const history = db.prepare('SELECT * FROM project_supply_requests_history WHERE supply_id = ? ORDER BY created_at ASC').all(req.params.id);
  ...
  router.post('/', projectSupply, (req, res) => {
    try {
      ...
      const result = db.prepare(`
        INSERT INTO project_supply_requests (user_id, request_number, project_name, items, description, urgency, estimated_cost, deadline, department_id, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(req.user.id, requestNumber, project_name, JSON.stringify(items), description || '', urgency, estimated_cost || null, deadline || null, user?.department_id, 'pending_supervisor');

      addHistory(result.lastInsertRowid, req.user.id, req.user.full_name, 'ثبت درخواست', null);

      if (supervisorId) {
        notify(supervisorId, 'درخواست تامین کالای پروژه جدید', `درخواست شماره ${requestNumber} توسط ${req.user.full_name} ثبت شد`, '/project-supply');
      }
  ...
  router.post('/:id/approve', (req, res) => {
    try {
      ...
      const request = db.prepare('SELECT * FROM project_supply_requests WHERE id = ?').get(req.params.id);
      if (!request) return res.status(404).json({ error: 'درخواست یافت نشد' });
      ...
      if (request.status === 'pending_supervisor') {
        ...
        db.prepare('UPDATE project_supply_requests SET supervisor_id = ?, supervisor_comment = ?, supervisor_date = ?, status = ? WHERE id = ?')
        ...
        managers.forEach(mgr => notify(mgr.id, 'درخواست تامین کالا نیاز به تایید', `درخواست شماره ${request.request_number} توسط سرپرست تایید شد`, '/project-supply'));
      } else if (request.status === 'pending_manager') {
        newStatus = 'approved';
        db.prepare('UPDATE project_supply_requests SET manager_id = ?, manager_comment = ?, manager_date = ?, status = ? WHERE id = ?')
          .run(req.user.id, comment, now, newStatus, req.params.id);
      ...
      notify(request.user_id, 'تایید درخواست تامین کالا', `درخواست شماره ${request.request_number} تایید شد`, '/project-supply');
  ...
  router.post('/:id/reject', (req, res) => {
    try {
      ...
      notify(request.user_id, 'رد درخواست تامین کالا', `درخواست شماره ${request.request_number} رد شد`, '/project-supply');
  ...
  router.delete('/:id', (req, res) => {
    try {
      ...
      db.prepare('DELETE FROM project_supply_requests_history WHERE supply_id = ?').run(req.params.id);
      db.prepare('DELETE FROM project_supply_requests WHERE id = ?').run(req.params.id);
