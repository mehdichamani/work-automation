...
async function getConfig() {
  try {
    const row = await prisma.setting.findUnique({ where: { key: 'camera_config' } });
...
async function saveConfig(config) {
  await prisma.setting.upsert({
    where: { key: 'camera_config' },
    update: { value: JSON.stringify(config) },
    create: { key: 'camera_config', value: JSON.stringify(config) },
  });
}
...
module.exports = function () {
  ...
  router.get('/config', roleGuard('admin'), async (req, res) => {
    try {
  ...
  router.get('/test', roleGuard('admin'), async (req, res) => {
    const config = await getConfig();
