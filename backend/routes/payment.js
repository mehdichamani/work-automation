...
const { validateInput, payment } = require('../middleware/validate');
...
module.exports = function (db) {
  ...
  function addHistory(id, userId, userName, action, comment) { addHistoryHelper(db, 'payment_history', 'request_id', id, userId, userName, action, comment); }
  function findSupervisorId(departmentId) { return findSupervisorIdHelper(db, departmentId); }
  function getNextNumber() { return getNextNumberHelper(db, 'payment_counter', 'وجه'); }
  ...
  router.get('/:id', (req, res) => {
    try {
      ...
      const history = db.prepare('SELECT * FROM payment_history WHERE request_id = ? ORDER BY created_at ASC').all(req.params.id);
  ...
  router.post('/', validateInput({ title: 200, description: 1000 }), payment, (req, res) => {
    try {
      const { amount, payment_type, description, reason, recipient_name, bank_name, card_number } = req.body;
      if (!amount || !payment_type) {
        return res.status(400).json({ error: 'مبلغ و نوع پرداخت الزامی است' });
      }
      ...
      const result = db.prepare(`
        INSERT INTO payment_requests (user_id, request_number, amount, payment_type, description, reason, recipient_name, bank_name, card_number, department_id, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(req.user.id, requestNumber, amount, payment_type, description || '', reason || '', recipient_name || '', bank_name || '', card_number || '', user?.department_id, 'pending_supervisor');

      addHistory(result.lastInsertRowid, req.user.id, req.user.full_name, 'ثبت درخواست', null);

      if (supervisorId) {
        notify(supervisorId, 'درخواست وجه جدید', `درخواست شماره ${requestNumber} به مبلغ ${parseInt(amount).toLocaleString()} تومان ثبت شد`, '/payment');
      }
  ...
  router.post('/:id/approve', (req, res) => {
    try {
      ...
      if (request.status === 'pending_supervisor') {
        ...
        db.prepare('UPDATE payment_requests SET supervisor_id = ?, supervisor_comment = ?, supervisor_date = ?, status = ? WHERE id = ?')
        ...
        managers.forEach(mgr => notify(mgr.id, 'درخواست وجه نیاز به تایید', `درخواست شماره ${request.request_number} توسط سرپرست تایید شد`, '/payment'));
      } else if (request.status === 'pending_manager') {
        newStatus = 'approved';
        db.prepare('UPDATE payment_requests SET manager_id = ?, manager_comment = ?, manager_date = ?, status = ? WHERE id = ?')
          .run(req.user.id, comment, now, newStatus, req.params.id);
      ...
      notify(request.user_id, 'تایید درخواست وجه', `درخواست شماره ${request.request_number} تایید شد`, '/payment');
  ...
  router.post('/:id/reject', (req, res) => {
    try {
      ...
      notify(request.user_id, 'رد درخواست وجه', `درخواست شماره ${request.request_number} رد شد`, '/payment');
  ...
  router.delete('/:id', (req, res) => {
    try {
      ...
      db.prepare('DELETE FROM payment_history WHERE request_id = ?').run(req.params.id);
