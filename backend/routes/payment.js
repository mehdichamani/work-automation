...
module.exports = function () {
  ...
  async function addHistory(id, userId, userName, action, comment) { await addHistoryHelper('payment_history', 'request_id', id, userId, userName, action, comment); }
  async function findSupervisorId(departmentId) { return findSupervisorIdHelper(departmentId); }
  async function getNextNumber() { return getNextNumberHelper('payment_counter', 'وجه'); }
  ...
  router.post('/', validateInput({ title: 200, description: 1000 }), payment, async (req, res) => {
    try {
      ...
      if (supervisorId) {
        await notify(supervisorId, 'درخواست وجه جدید', `درخواست شماره ${requestNumber} به مبلغ ${parseInt(amount).toLocaleString()} تومان ثبت شد`, '/payment');
      }
  ...
  router.post('/:id/approve', async (req, res) => {
    try {
      ...
      const request = await prisma.paymentRequest.findUnique({ where: { id: Number(req.params.id) } });
      if (!request) return res.status(404).json({ error: 'درخواست یافت نشد' });
      ...
      if (request.status === 'pending_supervisor') {
        ...
        for (const mgr of managers) {
          await notify(mgr.id, 'درخواست وجه نیاز به تایید', `درخواست شماره ${request.request_number} توسط سرپرست تایید شد`, '/payment');
        }
      ...
      await notify(request.userId, 'تایید درخواست وجه', `درخواست شماره ${request.request_number} تایید شد`, '/payment');
  ...
  router.post('/:id/reject', async (req, res) => {
    try {
      ...
      await notify(request.userId, 'رد درخواست وجه', `درخواست شماره ${request.request_number} رد شد`, '/payment');
  ...
  router.delete('/:id', async (req, res) => {
    try {
      ...
      await prisma.paymentHistory.deleteMany({ where: { requestId: Number(req.params.id) } });
      await prisma.paymentRequest.delete({ where: { id: Number(req.params.id) } });
