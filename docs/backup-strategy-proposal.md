...
* **فایل تغییریافته:** نسخه موجود در مقصد **ابتدا تغییر نام می‌گیرد** (با پسوند تاریخ/نسخه، مثلاً `file.pdf` → `file.2026-08-24T083200.pdf`) تا با فایل جدید تداخل نداشته باشد، سپس نسخه جدید در مسیر اصلی کپی می‌شود. به این ترتیب هیچ نسخه‌ای از بین نمی‌رود و تاریخچه تغییرات حفظ می‌شود.
...
flowchart TD
    ...
    HashCheck -- بله --> RenameOld[تغییر نام نسخه قدیمی در مقصد\nمثال: file.pdf → file.2026-08-24T083200.pdf]
    RenameOld --> CopyUpdated[کپی نسخه جدید در مسیر اصلی مقصد]
    HashCheck -- خیر --> Skip[رد شدن بدون مصرف پهنای باند و دیسک]
    
    CopyNew --> NextFile[فایل بعدی]
    CopyUpdated --> NextFile
    Skip --> NextFile
