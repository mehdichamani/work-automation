...
export function printLetter(letter, history = []) {
  const html = `
    ...
    <body>
      ...
      <div class="meta">
        <div class="meta-item"><span class="meta-label">شماره:</span><span class="meta-value">${letter.letter_number || '-'}</span></div>
      ...
      <div class="meta" style="margin-bottom: 15px;">
        <div class="meta-item"><span class="meta-label">فرستنده:</span><span class="meta-value">${letter.sender_name || '-'} (${letter.sender_unit_name || '-'})</span></div>
        ${letter.manager_name ? `<div class="meta-item"><span class="meta-label">مدیر بررسی‌کننده:</span><span class="meta-value">${letter.manager_name}</span></div>` : ''}
      </div>
      
      <div class="subject">موضوع: ${letter.subject || '-'}</div>
      
      ${letter.body ? `<div class="body">${letter.body}</div>` : ''}
      
      ${letter.attachment_name ? `<div class="attachment">فایل پیوست: ${letter.attachment_name}</div>` : ''}
      
      ${letter.manager_comment ? `<div style="margin-bottom: 15px; padding: 10px; background: #eff6ff; border-radius: 8px; font-size: 12px;"><strong>نظر مدیر:</strong> ${letter.manager_comment}</div>` : ''}
  ...
  const html = `
    ...
    <body>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:20px;">
          ...
          <div>
            ...
            <div style="margin-top:6px;font-size:11px;color:#374151;">
              ${data.application_number ? `<span style="margin-left:15px;"><strong>شماره:</strong> ${data.application_number}</span>` : ''}
              <span><strong>تاریخ ثبت:</strong> ${data.created_at ? new Date(data.created_at).toLocaleDateString('fa-IR') : new Date().toLocaleDateString('fa-IR')}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <div class="fields">
          <div class="field"><span class="field-label">نام و نام خانوادگی</span><span class="field-value">${data.full_name || ''}</span></div>
          <div class="field"><span class="field-label">نام پدر</span><span class="field-value">${data.father_name || ''}</span></div>
          <div class="field"><span class="field-label">شماره شناسنامه</span><span class="field-value">${data.national_id || ''}</span></div>
          <div class="field"><span class="field-label">صادره از</span><span class="field-value">${data.national_id_issued_from || ''}</span></div>
          <div class="field"><span class="field-label">تاریخ تولد</span><span class="field-value">${data.birth_date || ''}</span></div>
          <div class="field"><span class="field-label">محل تولد</span><span class="field-value">${data.birth_place || ''}</span></div>
          <div class="field"><span class="field-label">مدت اقامت</span><span class="field-value">${data.residence_duration || ''}</span></div>
          <div class="field"><span class="field-label">ملیت</span><span class="field-value">${data.nationality || ''}</span></div>
          <div class="field"><span class="field-label">مذهب</span><span class="field-value">${data.religion || ''}</span></div>
          <div class="field"><span class="field-label">زبان</span><span class="field-value">${data.language || ''}</span></div>
          <div class="field"><span class="field-label">آخرین مدرک تحصیلی</span><span class="field-value">${data.education_level || ''}</span></div>
          <div class="field"><span class="field-label">محل تحصیل</span><span class="field-value">${data.education_place || ''}</span></div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <div class="fields">
          ...
          <div class="field"><span class="field-label">رسته خدمت</span><span class="field-value">${data.military_service_type || ''}</span></div>
          <div class="field"><span class="field-label">معافیت غیر پزشکی</span><span class="field-value">${data.military_exempt_non_medical || ''}</span></div>
          <div class="field"><span class="field-label">معافیت پزشکی</span><span class="field-value">${data.military_exempt_medical || ''}</span></div>
          <div class="field field-full"><span class="field-label">علت معافیت</span><span class="field-value">${data.military_exempt_reason || ''}</span></div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <div class="fields">
          <div class="field"><span class="field-label">وضعیت تأهل</span><span class="field-value">${data.marital_status || ''}</span></div>
          <div class="field"><span class="field-label">تعداد فرزندان</span><span class="field-value">${data.children_count || '0'}</span></div>
          <div class="field"><span class="field-label">شغل همسر</span><span class="field-value">${data.spouse_job || ''}</span></div>
      ...
      <div class="section">
        ...
        <div class="fields">
          <div class="field"><span class="field-label">وضعیت مسکن</span><span class="field-value">${data.housing_status || ''}</span></div>
          ...
          <div class="field"><span class="field-label">شماره تلفن تماس</span><span class="field-value">${data.phone_number || ''}</span></div>
          <div class="field field-span3"><span class="field-label">نشانی محل سکونت</span><span class="field-value">${data.residential_address || ''}</span></div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <div class="fields">
          <div class="field field-span3"><span class="field-label">ویژگی‌های اخلاقی (۳ مورد)</span><div class="textarea-box">${data.moral_traits || ''}</div></div>
          <div class="field">
          ...
          <div class="field field-full"><span class="field-label">نام و نوع نسبت</span><span class="field-value">${data.relatives_details || ''}</span></div>
          <div class="field">
          ...
          <div class="field field-full"><span class="field-label">سابقه اشتغال در کارخانجات کاوه</span><span class="field-value">${data.kave_factories || ''}</span></div>
          <div class="field">
          ...
          <div class="field field-full"><span class="field-label">مدت زمان استعمال</span><span class="field-value">${data.smoking_duration || ''}</span></div>
        </div>
      </div>
      ...
      <div class="section">
        <div class="fields">
          <div class="field field-span3">
            ...
            <div class="textarea-box">${data.foreign_languages || ''}</div>
          </div>
          <div class="field">
          ...
          <div class="field field-span3">
            ...
            <div class="textarea-box">${data.computer_skills || ''}</div>
          </div>
          <div class="field field-span3">
            <span class="field-label">دوره‌های آموزشی</span>
            <div class="textarea-box">${data.training_courses || ''}</div>
          </div>
          <div class="field field-span3">
            <span class="field-label">معرف‌ها</span>
            <div class="textarea-box">${data.references_info || ''}</div>
          </div>
        </div>
      </div>
