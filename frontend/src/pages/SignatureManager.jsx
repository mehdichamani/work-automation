...
export default function SignatureManager() {
  ...
  const loadLogs = async () => {
    try {
      setLogsLoading(true);
      const res = await api.get('/signature/admin/logs');
      setSignLogs(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      toast.error('خطا در دریافت تاریخچه ورود امضاها');
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in p-2 md:p-4">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 pb-2">
        ...
        >
          <span>📜</span>
          <span>تاریخچه و لاگ ورود گروهی</span>
        </button>
      </div>

      {/* ───────────────────────────────────────────────────────────── */}
      {/* TAB 1: PERSONNEL LIST (ADMIN OVERVIEW)                         */}
      {/* ───────────────────────────────────────────────────────────── */}
      {activeTab === 'admin-overview' && (
        <div className="space-y-4">
          {/* Filters & Search Toolbar */}
          <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-3">
            {/* Search */}
            <div className="relative w-full md:w-80">
              <input
                type="text"
                placeholder="جستجو با نام، کد پرسنلی یا واحد..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-9 pr-4 py-2.5 rounded-2xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm"
              />
            ...
            <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
              {/* Status Segmented */}
              <div className="bg-gray-100 p-1 rounded-2xl flex text-xs font-semibold">
                <button
                ...
                >
                  همه ({personnelList.length})
                </button>
                <button
                  onClick={() => setFilterStatus('has_sig')}
                  className={`px-3 py-1.5 rounded-xl transition-all ${
                    filterStatus === 'has_sig'
                      ? 'bg-white text-emerald-700 shadow-sm font-bold'
                      : 'text-gray-500 hover:text-emerald-700'
                  }`}
                >
                  دارای امضا ({stats.signed})
                </button>
                <button
                  onClick={() => setFilterStatus('no_sig')}
                  className={`px-3 py-1.5 rounded-xl transition-all ${
                    filterStatus === 'no_sig'
              ...
              <select
                ...
                className="py-2 px-3 rounded-2xl border border-gray-200 bg-white text-xs font-medium focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
          ...
          {personnelLoading ? (
            <div className="bg-white rounded-3xl p-12 text-center text-gray-400 border border-gray-100 shadow-sm">
              ...
              <p className="text-sm font-medium">در حال دریافت لیست پرسنل...</p>
            </div>
          ) : filteredPersonnel.length === 0 ? (
            <div className="bg-white rounded-3xl p-12 text-center text-gray-400 border border-gray-100 shadow-sm">
              <p className="text-4xl mb-2">👤</p>
              <p className="text-sm font-medium">پرسنلی با شرایط انتخاب شده یافت نشد</p>
            </div>
          ) : (
                ...
                <table className="w-full text-right text-xs">
                  <thead className="bg-gray-50/80 text-gray-600 font-bold border-b border-gray-100">
                    <tr>
                      ...
                      <th className="py-3.5 px-4">واحد / دپارتمان</th>
                      <th className="py-3.5 px-4">نقش</th>
                      <th className="py-3.5 px-4 text-center">وضعیت امضا</th>
                      <th className="py-3.5 px-4 text-center">پیش‌نمایش امضا</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredPersonnel.map((user) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-center">
                          {user.has_signature ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                              ...
                              ثبت شده ({user.signature_type === 'scanned' ? 'اسکن' : 'ترسیمی'})
                            </span>
                          ) : (
                        ...
                        <td className="py-3 px-4 text-center">
                          {user.has_signature ? (
                            <div className="inline-block p-1 bg-gray-50 border border-gray-200 rounded-xl hover:scale-110 transition-transform">
                              {user.scanned_signature ? (
                                <img
                                  src={user.scanned_signature}
                                  alt={user.full_name}
                                  className="h-9 w-20 object-contain mx-auto"
                                />
                              ) : user.signature_data ? (
                                <img
                        ...
                        <td className="py-3 px-4 text-center text-gray-400 font-mono text-[11px]">
                          {user.signature_updated_at || user.signature_created_at
                            ? toJalaliDateTime(user.signature_updated_at || user.signature_created_at)
                            : '-'}
                        </td>
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-xl text-xs font-bold bg-primary-50 text-primary-700 hover:bg-primary-100 transition-colors flex items-center gap-1"
                              title="ثبت یا تغییر امضا"
                            >
                              <span>✏️</span>
                              <span>{user.has_signature ? 'ویرایش' : 'ثبت امضا'}</span>
                            </button>
                            {user.has_signature && (
                              <button
                                onClick={() => handleDeleteSignature(user)}
                                className="p-1.5 rounded-xl text-xs text-red-500 hover:bg-red-50 transition-colors"
                                title="حذف امضا"
                              >
                                🗑️
                              </button>
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {/* TAB 3: LOGS & HISTORY OF BULK UPLOADS                          */}
      {/* ───────────────────────────────────────────────────────────── */}
      {activeTab === 'logs' && (
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-gray-100">
            <div>
              <h3 className="font-bold text-gray-800 text-base flex items-center gap-2">
                <span>📜</span>
                <span>لاگ و تاریخچه ورود گروهی امضاها</span>
              </h3>
              <p className="text-xs text-gray-500 mt-1">
                ثبت ردپای دقیق عملیات بارگذاری دسته‌جمعی امضا، زمان، کاربر انجام‌دهنده و کدهای پرسنلی بروزرسانی‌شده
              </p>
            </div>
            <button
              onClick={loadLogs}
              disabled={logsLoading}
              className="text-xs text-primary-600 hover:text-primary-800 flex items-center gap-1 font-medium bg-primary-50 px-3.5 py-2 rounded-xl hover:bg-primary-100 transition-colors self-start sm:self-auto"
            >
              <span>🔄</span>
              <span>بروزرسانی تاریخچه</span>
            </button>
          </div>
          ...
          {logsLoading ? (
            <div className="text-center py-16 text-gray-400">
              <div className="animate-spin text-3xl mb-2">⏳</div>
              <p className="text-sm font-medium">در حال بارگذاری تاریخچه...</p>
            </div>
          ) : signLogs.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              <p className="text-4xl mb-2">📁</p>
              <p className="text-sm font-medium">هیچ سابقه ورود گروهی امضایی در سیستم ثبت نشده است</p>
            </div>
          ) : (
            <div className="space-y-4">
              {signLogs.map((log) => (
                <div
                  key={log.id}
                  className="p-4 rounded-2xl border border-gray-100 bg-gray-50/60 hover:bg-gray-50 transition-colors space-y-3"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-3">
                      <span className="p-2.5 bg-indigo-100 text-indigo-700 rounded-2xl text-base font-bold shadow-sm">
                        📁
                      </span>
                      <div>
                        <p className="text-sm font-bold text-gray-800">
                          ورود گروهی امضا توسط{' '}
                          <span className="text-primary-700 font-extrabold">{log.user_full_name}</span>
                        </p>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="px-3 py-1 rounded-xl text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-200">
                        {log.details?.okCount || 0} فایل موفق
                      </span>
                      {(log.details?.failCount > 0) && (
                        <span className="px-3 py-1 rounded-xl text-xs font-bold bg-red-100 text-red-800 border border-red-200">
                          {log.details.failCount} خطا
                        </span>
                      )}
                      <span className="text-xs text-gray-500 bg-white px-2.5 py-1 rounded-xl border border-gray-200 font-mono">
                        مجموع: {log.details?.totalFiles || 0}
                      </span>
                    </div>
                  </div>

                  {/* Updated personnel badges */}
                  {log.details?.updatedPersonnel && log.details.updatedPersonnel.length > 0 && (
                    <div className="bg-white p-3.5 rounded-2xl border border-gray-100 space-y-2">
                      <span className="text-xs font-bold text-gray-700 flex items-center gap-1.5">
                        <span>👥</span>
                        <span>کدهای پرسنلی بروزرسانی‌شده با این ورود ({log.details.updatedPersonnel.length} نفر):</span>
                      </span>
                      <div className="flex flex-wrap gap-2 max-h-36 overflow-y-auto p-1">
                        {log.details.updatedPersonnel.map((p, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs bg-gray-50 text-gray-800 font-medium border border-gray-200 hover:border-primary-300 transition-colors"
                          >
                            <span className="font-bold text-primary-700 font-mono">{p.employeeCode}</span>
                            <span className="text-gray-300">|</span>
                  ...
                  {/* Failed files if any */}
                  {log.details?.failedFiles && log.details.failedFiles.length > 0 && (
                    <div className="bg-red-50/80 p-3 rounded-2xl border border-red-100 text-xs text-red-700 space-y-1.5">
                      <span className="font-bold flex items-center gap-1">
                        <span>⚠️</span>
                        <span>فایل‌های پردازش نشده یا دارای خطا:</span>
                      </span>
                      <div className="flex flex-wrap gap-2">
                        {log.details.failedFiles.map((f, fIdx) => (
                          <span key={fIdx} className="bg-white px-2.5 py-1 rounded-lg border border-red-200 font-mono text-[11px]">
                            {f.file}: <span className="font-sans font-medium text-red-600">{f.message}</span>
                          </span>
                        ))}
                      </div>
                    </div>
